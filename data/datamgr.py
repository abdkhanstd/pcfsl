...
class SimpleDataManager(DataManager):
    ...
    def get_data_loader(self, data_file, aug): #parameters that would change on train/val set
        ...
        data_loader_params = dict(batch_size = self.batch_size, shuffle = True, num_workers = 1, pin_memory = True,drop_last=True)
        data_loader = torch.utils.data.DataLoader(dataset, **data_loader_params)
        return data_loader

class SetDataManager(DataManager):
    ## dataloader of standard meta learning, n-way k-shot for each episode
    def __init__(self, n_way, n_support, n_query, n_eposide =400):        
        super(SetDataManager, self).__init__()
        self.n_way = n_way
        self.batch_size = n_support + n_query
        from io_utils import parse_args
        params=parse_args('train')
        self.n_eposide = params.num_episode
...
if __name__=='__main__':
    # data_loader_params = dict(batch_sampler = 1,  num_workers = 1, pin_memory = True)
    ...
    base_name = os.path.abspath(os.path.join(pwd,'../..','dataset/'+params.dataset+'/base.json')) 
    datasets = SetDataManager(5, 5, 15, n_eposide=params.num_spisode).get_data_loader(base_name, True)
