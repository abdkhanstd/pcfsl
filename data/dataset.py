...
class SimpleDataset:
    ...
    def __init__(self, data_file, npoints=2048,preprocess=False,transform=None):
        ...
        self.save_path = save_path = os.path.abspath(self.datafile+'/../'+save_path+'.dat')
        print("###########",save_path,data_file)
        if self.preprocess:
...
class SetDataset:
    ...
    def __init__(self, data_file, batch_size, transform=None):
        ...
        save_path = os.path.abspath(data_file+'/../'+save_path+'.dat')
        print("--> Using saved points from file: ",save_path)
...
if __name__=='__main__':
    ...
    base_name = os.path.abspath(os.path.join(pwd,'../..','dataset/'+params.dataset+'/novel.json')) 
    dataset = SimpleDataset(base_name,preprocess=True)
